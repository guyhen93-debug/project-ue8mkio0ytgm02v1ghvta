...
const AdminPanel: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [ordersData, clientsData, sitesData, productsData] = await Promise.all([
                    ...
                    <CardContent className="p-4">
                      <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
                        <div className={cn(
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-1 text-xs md:text-sm text-gray-600">
                            <p>{t('customer')}: {order.created_by}</p>
                            <p>{t('client')}: {order.client_name}</p>
                            <p>{t('site')}: {order.site_name}</p>
                            <p>{t('product')}: {t(order.product_id)} - {order.quantity_tons} {t('tons')}</p>
                        ...
                        <div className="flex flex-col gap-2 w-full lg:w-auto min-w-0 lg:min-w-[200px]">
                    ...
                    <Card key={site.id}>
                      <CardContent className="p-4">
                        <div className={cn(
                          ...
                          <div className="flex-1">
                            <h3 className="font-semibold text-base md:text-lg">{site.site_name}</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-1 text-sm text-gray-600 mt-2">
                              <p>{t('client')}: {client?.name || 'Unknown'}</p>
                              <p>{t('region')}: {t(site.region_type)}</p>
                              <p className="md:col-span-2">{t('contact')}: {site.contact_name} - {site.contact_phone}</p>
                              <p>{t('status')}: {site.is_active ? t('active') : t('inactive')}</p>
                            </div>
                          </div>
                          <div className="flex gap-2 w-full sm:w-auto">
                            <Button 
            ...
            <DialogContent className={cn(
              ...
              <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-end border-t pt-4">
                  ...
                  <Button onClick={() => saveOrder(editingOrder)} className="w-full sm:w-auto">
                    {t('save')}
                  </Button>
                </div>
              </div>
            ...
            <DialogContent className={cn(
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-end">
                  ...
                  <Button onClick={() => saveClient(editingClient)} className="w-full sm:w-auto">
                    {t('save')}
                  </Button>
                </div>
              </div>
          ...
          <Dialog open={!!editingSite} onOpenChange={() => setEditingSite(null)}>
            <DialogContent className={cn(
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-end">
                  <Button variant="outline" onClick={() => setEditingSite(null)} className="w-full sm:w-auto">
                    {t('cancel')}
                  </Button>
                  <Button onClick={() => saveSite(editingSite)} className="w-full sm:w-auto">
                    {t('save')}
                  </Button>
                </div>
              </div>
